#!/bin/bash
# Setup CPU-only environment for Query Cache Service
# This script configures environment variables and tests the setup

set -e  # Exit on any error

echo "Setting up CPU-only environment for Query Cache Service..."

# Create environment setup script
cat > cpu_environment.sh << 'EOF'
#!/bin/bash
...
chmod +x cpu_environment.sh

# Source the environment for this session
source cpu_environment.sh

# Create a test script to verify CPU-only setup
cat > test_cpu_setup.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x test_cpu_setup.py

echo ""
echo "✓ CPU-only environment setup complete!"
echo ""
echo "Files created:"
echo "  - cpu_environment.sh (source this before running the service)"
echo "  - test_cpu_setup.py (test script to verify setup)"
echo ""
echo "Usage:"
echo "  1. Source environment: source cpu_environment.sh"
echo "  2. Test setup: python test_cpu_setup.py"
echo "  3. Start service: python -m src.api.server"
echo ""
echo "The environment is now configured for CPU-only operation."